...
describe('Things Endpoints', function() {
  ...
  describe(`Given any endpoint, missing a basic token responds appropriately`, () => {
    ...
    const protectedEndpoints = [
      ...
      {name:`POST /api/reviews`, path:`/api/reviews`},
    ]

    protectedEndpoints.forEach(endpoint => {
      describe(endpoint.name, () =>{
        it(`responds with 401 when no basic token`, () => {
          return supertest(app)
            .get(endpoint.path)
            .expect(401, { error: `Missing basic token` })
        })

        it(`responds with 401 when no credentials in token`, ()=> {
        ...
        it(`responds with 401 when invalid user`,() => {
  ...
  describe(`GET /api/things`, () => {
    context(`Given no things`, () => {
      it(`responds with 200 and an empty list`, () => {
        return supertest(app)
          .get('/api/things')
          .set('Authorization',helpers.makeAuthHeader(testThings[0]))
          .expect(200, [])
      })
    })
